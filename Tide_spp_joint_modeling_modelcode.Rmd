...
### m7 TOD, Month, Limb
```{r}
d7 <- df[c("detections", "logdays", "ID", "Species", "sin_tod", "cos_tod", "sin_mnth", "cos_mnth", "minoff", "limb")]
m7prep <- map2stan(
  alist(
    detections ~ dzipois(p , lambda ),
    logit(p) <- a0 + bm0*minoff,
    log(lambda) <- a + logdays + a_id[ID] + a_species[Species] + bst*sin_tod + bct*cos_tod + bsm*sin_mnth + bcm*cos_mnth + bl*limb ,
...
### m8 Stage, Limb
```{r}
d8 <- df[c("detections", "logdays", "ID", "Species", "stages", "minoff", "limb")]
m8prep <- map2stan(
  alist(
    detections ~ dzipois(p , lambda ),
    logit(p) <- a0 + bm0*minoff ,
    log(lambda) <- a + logdays + a_id[ID] + a_species[Species] + bs*stages + bl*limb ,
...
### m9 Drange, Limb
```{r}
d9 <- df[c("detections", "logdays", "ID", "Species", "dranges", "minoff", "limb")]
m9prep <- map2stan(
  alist(
    detections ~ dzipois(p , lambda ),
    logit(p) <- a0 + bm0*minoff ,
    log(lambda) <- a + logdays + a_id[ID] + a_species[Species] + bd*dranges + bl*limb ,
...
### m10 TOD, Stage, Limb
```{r}
d10 <- df[c("detections", "logdays", "ID", "Species", "stages", "limb", "sin_tod", "cos_tod", "minoff")]
m10prep <- map2stan(
  alist(
    detections ~ dzipois(p , lambda ),
    logit(p) <- a0 + bm0*minoff,
    log(lambda) <- a + logdays + a_id[ID] + a_species[Species] + bg*stages + bl*limb + bst*sin_tod + bct*cos_tod,
    c(a0, a, bg, bl, bst, bct, bm0) ~ dnorm(0,1) ,
    a_id[ID] ~ dnorm( 0 , sigma_id ) ,
    a_species[Species] ~ dnorm( 0 , sigma_species ) ,
    sigma_id ~ dcauchy(0,1) ,
    sigma_species ~ dcauchy(0,1) 
...
### m11 TOD, Drange, Limb
```{r}
d11 <- df[c("detections", "logdays", "ID", "Species", "dranges", "limb", "sin_tod", "cos_tod", "minoff")]
m11prep <- map2stan(
  alist(
    detections ~ dzipois(p , lambda ),
    logit(p) <- a0 + bm0*minoff,
    log(lambda) <- a + logdays + a_id[ID] + a_species[Species] + bd*dranges + bl*limb + bs*sin_tod + bc*cos_tod,
    c(a0, a, bd, bl, bs, bc, bm0) ~ dnorm(0,1) ,
    a_id[ID] ~ dnorm( 0 , sigma_id ) ,
    a_species[Species] ~ dnorm( 0 , sigma_species ) ,
    sigma_id ~ dcauchy(0,1) ,
    sigma_species ~ dcauchy(0,1) 
...
### m12 PAR, Stage, Limb
```{r}
d12 <- df[c("detections", "logdays", "ID", "Species", "stages", "limb", "pars", "minoff")]
m12prep <- map2stan(
  alist(
    detections ~ dzipois(p , lambda ),
    logit(p) <- a0 + bm0*minoff,
    log(lambda) <- a + logdays + a_id[ID] + a_species[Species] + bg*stages + bl*limb + bp*pars,
    c(a0, a, bg, bl, bp, bm0) ~ dnorm(0,10) ,
    a_id[ID] ~ dnorm( 0 , sigma_id ) ,
    a_species[Species] ~ dnorm( 0 , sigma_species ) ,
    sigma_id ~ dcauchy(0,1) ,
    sigma_species ~ dcauchy(0,1) 
...
### m13 PAR, Drange, Limb
```{r}
d13 <- df[c("detections", "logdays", "ID", "Species", "dranges", "limb", "pars", "minoff")]
m13prep <- map2stan(
  alist(
    detections ~ dzipois(p , lambda ) ,
    logit(p) <- a0 + bm0*minoff ,
    log(lambda) <- a + logdays + a_id[ID] + a_species[Species] + bd*dranges + bl*limb + bp*pars ,
