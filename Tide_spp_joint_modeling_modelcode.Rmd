...
m10prep <- map2stan(
  alist(
    detections ~ dzipois(p , lambda ),
    logit(p) <- a0 + bm0*minoff,
    log(lambda) <- a + logdays + a_id[ID] + a_species[Species] + bs*stages + bl*limb + bst*sin_tod + bct*cos_tod,
...
m12prep <- map2stan(
  alist(
    detections ~ dzipois(p , lambda ),
    logit(p) <- a0 + bm0*minoff,
    log(lambda) <- a + logdays + a_id[ID] + a_species[Species] + bs*stages + bl*limb + bp*pars,
